...
export function WABtn({ text = 'Book Appointment', className = '', large = false }: { text?: string; className?: string; large?: boolean }) {
  return (
    <motion.a href={WA} target="_blank" rel="noopener noreferrer"
      className={`inline-flex items-center gap-2 bg-[#25D366] text-white font-semibold rounded-full shadow-lg hover:shadow-xl hover:bg-[#20BD5A] transition-all btn-shimmer ${large ? 'px-8 py-4 text-lg' : 'px-6 py-3 text-sm'} ${className}`}
      whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
      <svg viewBox="0 0 24 24" className={large ? 'w-6 h-6' : 'w-5 h-5'} fill="currentColor">
